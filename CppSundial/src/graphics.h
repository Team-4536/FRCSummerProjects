...
/*
void loadOBJMesh(const char* path, gfx_VertexArray* outVB, gfx_IndexBuffer* outIB) {
    ...
    std::vector<std::array<u32, 9>> f = std::vector<std::array<u32, 9>>();
    ...
    while (t) {
	   ...
	   //skip comments + other things we don't understand
	   if (line.length() == 0) { continue; }
	   else if (line.at(0) == '#') { continue; }
	   else if (line.at(0) == 'o') { continue; }
	   else if (line.at(0) == 's') { continue; }


	   auto splitString = [](std::string s, std::vector<std::string>* out, char del) -> void {
		  //split strings to make parsing out values easy
		  size_t pos = 0;
		  size_t lastPos = -1;
		  out->reserve(4);
		  while ((pos = s.find(del, lastPos + 1)) != std::string::npos) {
			 out->push_back(s.substr(lastPos+1, pos - lastPos-1));
			 lastPos = pos;
		  }
		  out->push_back(s.substr(lastPos + 1));
	   };
	   ...
	   if (split[0] == "v") {
		  SAT_ASSERT(split.size() == 4);

		  glm::vec3 vert = glm::vec3(
			 std::strtof(split[1].c_str(), nullptr),
			 std::strtof(split[2].c_str(), nullptr),
			 std::strtof(split[3].c_str(), nullptr));

		  v.push_back(vert);
	   }
	   else if (split[0] == "vt") {
		  SAT_ASSERT(split.size() == 3);

		  glm::vec2 vert = glm::vec2(
			 std::strtof(split[1].c_str(), nullptr),
			 std::strtof(split[2].c_str(), nullptr));
		  ...
		  splitString(split[1], &s, '/');
		  splitString(split[2], &s, '/');
		  splitString(split[3], &s, '/');

		  std::array<u32, 9> a = std::array<u32, 9>();
		  for (int i = 0; i < 9; i++) {
			 a[i] = (u32)strtol(s[i].c_str(), nullptr, 10); }
    ...
    const u32 vertSize = 3 + 3 + 2;
    u32 vbCount = (vertSize) * 3 * f.size();
    u32 vbSize = vbCount * sizeof(float);
    float* vbData = new float[vbSize];

    u32 ibCount = (3) * f.size();
    u32 ibSize = ibCount * sizeof(u32);
    u32* ibData = new u32[ibSize];

    //pos, uv, normal
    //this loop goes over all indicies, adds their data to vertData
    for (int i = 0; i < f.size(); i++) {
	   //note that j goes by threes, so it pointss at the right idx in the list of all things in the tri
	   for (int j = 0; j < 3; j++) {

		  u32 vertIdx = ((i * 3) + j) * vertSize;

		  // literally who did one-based indexing
		  u32 posIdx = f[i][j*3] - 1;
		  vbData[vertIdx + 0] = v[posIdx].x;
		  vbData[vertIdx + 1] = v[posIdx].y;
		  vbData[vertIdx + 2] = v[posIdx].z;

		  u32 texIdx = f[i][(j*3)+1] - 1;
		  vbData[vertIdx + 6] = vt[texIdx].x;
		  vbData[vertIdx + 7] = vt[texIdx].y;

		  u32 normalIdx = f[i][(j*3)+2] - 1;
		  vbData[vertIdx + 3] = vn[normalIdx].x;
		  vbData[vertIdx + 4] = vn[normalIdx].y;
		  vbData[vertIdx + 5] = vn[normalIdx].z;

		  //idx = current vert idx
		  ibData[i*3 + j] = i*3 + j;
	   }
    }

    gfx_registerVertexArray(gfx_vtype_POS3F_UV);

    setVertexbuffer(vb, vbData, vbSize);
    setIndexBuffer(ib, ibData, ibSize);
};
